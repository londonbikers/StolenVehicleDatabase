...
namespace SVDWebsite.Models
{
    public class AddVehicleModel
    {
        ...
        public SelectList VehicleTypes
        {
            get
            {
                var list = new SelectList(SVD.Controller.Instance.MakesAndModelsController.VehicleTypes, "Id", "Name");
                return list;
            }
        }
        ...
        public SelectList VehicleManufacturers
        {
            get
            {
                ...
                var makes = SVD.Controller.Instance.MakesAndModelsController.VehicleManufacturers.Where(q => q.Manufacturer.Types.Any(q2 => q2.Id == type.Id)).Select(q => q.Manufacturer);
        ...
        public SelectList VehicleModels
        {
            get
            {
                ...
                VehicleManufacturer make = null;
                if (!string.IsNullOrEmpty(VehicleManufacturerId))
                {
                    var manufacturerWrapper = SVD.Controller.Instance.MakesAndModelsController.GetManufacturer(int.Parse(VehicleManufacturerId));
                    if (manufacturerWrapper != null)
                        make = manufacturerWrapper.Manufacturer;
                }
                else
                {
                    var manufacturerWrapper = SVD.Controller.Instance.MakesAndModelsController.VehicleManufacturers.FirstOrDefault(q => q.Manufacturer.Types.Any(q2 => q2.Id == type.Id));
                    if (manufacturerWrapper != null)
                        make = manufacturerWrapper.Manufacturer;
                }
                
                var models = SVD.Controller.Instance.MakesAndModelsController.VehicleModels.Where(q => q.Model.ManufacturerId == make.Id).Select(q => q.Model);
        ...
        [StringLength(17, ErrorMessage = "VIN numbers are 17 characters long, please correct yours.")]
        [DataType(DataType.Text)]
        public string Vin { get; set; }

        [Display(Name = "Engine number")]
        [StringLength(17, ErrorMessage = "Engine numbers are 17 characters long, please correct yours.")]
        [DataType(DataType.Text)]
